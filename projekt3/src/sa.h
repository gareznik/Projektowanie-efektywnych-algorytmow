   ...
   // Upper Bound - RNN
    ...
    int computeMSTLowerBound() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (min_e[u] == 1e9) break;
            in_mst[u] = true;
            mst_cost += min_e[u];

            for (int v = 0; v < n; ++v) {
                if (!in_mst[v] && matrix[u][v] != -1) {
                    // przy ATSP bierzemy minimalną krawędź między (u,v) i (v,u)
    ...
    double calculateInitialTemperature(const std::vector<int>& initial_path, double init_prob = 0.99) {
        ...
        if (positive_deltas == 0) return 1000.0;
    ...
    int solve(int time_limit_s, int epoch_length, double alpha, int cooling_scheme, bool use_ub, double init_temp_param, std::vector<TracePoint>* trace = nullptr, int* out_initial_cost = nullptr) {
        auto start_time = std::chrono::high_resolution_clock::now();
        
        std::vector<int> current_path;
        if (use_ub) {
            current_path = generateRNNPath();
        } else {
